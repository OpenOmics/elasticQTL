...
**Why refit?** When the intersection set is smaller than the original model (e.g., because some variants are absent in validation cohorts), refitting allows the model to re-optimize coefficients on the available variants, potentially improving predictive performance.

**When to skip refit:**
- You want to use the exact original model weights
- The intersection set is identical or very similar to the original model variant set
- You're testing scoring infrastructure without modeling
